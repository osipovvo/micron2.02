		...
		if (stt && wtt)
		{
			...
			if ( (*counterTask)->transactions != NULL)
			{
				...
				if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
					res = MAYK_BeginWritingTariffMap( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
				else
				{
					res = ERROR_MEM_ERROR;
			...
			for( ; idx < sttLength ; idx++ )
			{
				if(res == OK)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							res = MAYK_WritingTariffMap_STT( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &stt[idx] , idx );
						else
						{
							res = ERROR_MEM_ERROR;
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			for(idx = 0 ; idx < wttLength ; idx++ )
			{
				if(res == OK)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							res = MAYK_WritingTariffMap_WTT( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &wtt[idx] , idx );
						else
						{
							res = ERROR_MEM_ERROR;
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			if(res == OK)
			{
				...
				if ( (*counterTask)->transactions != NULL)
				{
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						res = MAYK_WritingTariffMap_MTT( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &mtt );
					else
					{
						res = ERROR_MEM_ERROR;
						COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			if ( lwj.mdNumb > 0 )
			{
				if(res == OK)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							res = MAYK_WritingTariffMap_LWJ( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &lwj );
						else
						{
							res = ERROR_MEM_ERROR;
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			if(res == OK)
			{
				...
				if ( (*counterTask)->transactions != NULL)
				{
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						res = MAYK_EndWritingTariffMap( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
					else
					{
						res = ERROR_MEM_ERROR;
						COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			if ( maykIndication.loopCounter > 0 )
			{
				...
				if(res == OK)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdStartWritingIndications( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &maykIndication);
				...
				for ( ; loopIndex < maykIndication.loopCounter ; ++loopIndex)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdWritingIndicationsLoop( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , loopIndex , &maykIndication.loop[ loopIndex ]);
							(*counterTask)->indicationWritingTransactionCounter++;
						}
						else
						{
		...
		switch( parsingState )
		{
			...
			case SET_DEFAULT_RULES:
			{
				...
				if ( (*counterTask)->transactions != NULL)
				{
					...
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
					{
						res = MAYK_CmdSetPowerControlsDefault( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
					}
					else
					{
						COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskPowerOffOtherCases" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMasks1( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskGivenLevelsOvershoot" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMasks2( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskSwitchOffRelayAtEventUmax" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskUmax( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskSwitchOffRelayAtEventUmin" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskUmin( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskSwitchOffRelayAtEventImax" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskImax( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( !strncmp( blockParsingResult[ propertyIndex ].variable , "maskPowerOffPQ", strlen("maskPowerOffPQ") ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskPq( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskE30" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskE30( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskEDay" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskEd( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "maskEMonth" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetPowerMaskEm( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet);
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
				...
				if (powerLimitsFlag != 0)
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							...
							res = MAYK_SetPowerLimitsPq( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataToSet ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_E30:
			{
				...
				if ( e30LimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_E30 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_E30 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T1:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T1 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T1 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T2:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T2 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T2 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T3:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T3 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T3 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T4:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T4 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T4 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T5:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T5 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T5 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T6:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T6 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T6 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T7:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T7 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T7 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_T8:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T8 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_T8 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_ED_TS:
			{
				...
				if ( edLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_TS , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_ED_TS + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T1:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T1 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T1 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T2:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T2 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T2 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T3:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T3 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T3 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T4:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T4 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T4 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T5:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T5 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T5 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T6:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T6 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T6 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T7:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T7 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T7 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_T8:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T8 , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_T8 + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_LIMITS_EM_TS:
			{
				...
				if ( emLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_TS , &dataToSet[0] , 40 ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
					...
					if ( res == OK )
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_CmdSetPowerControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , MAYK_MCPCR_OFFSET_LIMITS_EM_TS + 40 , &dataToSet[40] , 40 ) ;
							}
							else
							{
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_POWER_SWITCH_UI:
			{
				...
				if ( uiLimitsFlag != 0 )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_SetPowerLimitsUI( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &dataToSet[0] ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							if ( strstr( blockParsingResult[ propertyIndex ].variable , "Power" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetTimingsMR_P( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , (unsigned int)mayk_PSM_Parametres.tP ) ;
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "Umin" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetTimingsUmin( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , (unsigned int)mayk_PSM_Parametres.tMin ) ;
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "UImax" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetTimingsUmax( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , (unsigned int)mayk_PSM_Parametres.tMax ) ;
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "delayBeforePowerOff" ) )
							{
								...
								if ( res == OK )
								{
									...
									if ( (*counterTask)->transactions != NULL)
									{
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = MAYK_SetTimingsDelayOff( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , (unsigned int)mayk_PSM_Parametres.tOff ) ;
										}
										else
										{
											COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
			...
			case PARSING_STATE_CAST_FINISH_CMD:
			{
				...
				if ( res == OK )
				{
					...
					if ( (*counterTask)->transactions != NULL)
					{
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = MAYK_CmdSetPowerControlsDone( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] ) ;
						}
						else
						{
							COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res);
				...
				if ( ( res == OK ) && ( powermanState >= 0 ) )
				{
					...
					{                                                                         
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) 
							{                                                                     
								res = MAYK_CmdSetPowerModuleSwitcherState( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , powermanState  );
							}                                                                     
							else                                                                  
							{                                                                     
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_WritePSM() exit with errorcode=%d", res); 
...
int MAYK_SyncCounterClock( counter_t * counter , counterTask_t ** counterTask )
{
	...
	if ( (abs(counter->syncWorth) > 1) && (abs(counter->syncWorth) <= (int)MAYK_SOFT_HARD_SYNC_SW))
	{
		...
		if ( (*counterTask)->transactions != NULL)
		{
			(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
			if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
				res = MAYK_SyncTimeSoft( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
			else
			{
				res = ERROR_MEM_ERROR;
				COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
	...
	else
	{
		...
		if ( (*counterTask)->transactions != NULL)
		{
			(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
			if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
				res = MAYK_SyncTimeHardy( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
			else
			{
				res = ERROR_MEM_ERROR;
				COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
		...
		switch(taskCreationState)
		{
				...
				if ( COMMON_CheckDtsForValid( &counter->currentDts ) == OK )
				{
					if (  allowSeasonFlg != counterAllowSeason )
					{
						...
						if ((*counterTask)->transactions != NULL) {
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = MAYK_CmdAllowSeasonChange (counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], allowSeasonFlg );
							else
								return ERROR_MEM_ERROR;
						} else
							return ERROR_MEM_ERROR;
				...
				else
				{
					...
					if ( needToAskFirmvareVersion == TRUE)
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = MAYK_CmdGetVersion( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
							else
							{
								res = ERROR_MEM_ERROR;
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			case TASK_CREATION_STATE_READ_RATIO_DATE_AND_TIME:
			{
				...
				if ( (*counterTask)->transactions != NULL)
				{
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						res = MAYK_GetDateAndTime( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
					else
					{
						res = ERROR_MEM_ERROR;
						COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
				...
				if (res == OK)
				{
					if (counter->ratio == 0xFF){
						...
						if ( (*counterTask)->transactions != NULL)
						{
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = MAYK_GetRatio( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
							else
							{
								res = ERROR_MEM_ERROR;
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			case TASK_CREATION_STATE_READ_CUR_METERAGES:
			{
				...
				if (res == OK)
				{
					...
					if ( STORAGE_CheckTimeToRequestMeterage( counter , STORAGE_CURRENT , &dtsToRequest, NULL ) == TRUE )
					{
						(*counterTask)->transactions = (counterTransaction_t **) realloc((*counterTask)->transactions, \
																						(++(*counterTask)->transactionsCount) * sizeof(counterTransaction_t *) );
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_GetCurrentMeterage( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , counter->maskTarifs);
							}
							else
							{
								res = ERROR_MEM_ERROR;
						...
						if ( (*counterTask)->transactions != NULL)
						{
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								res = MAYK_GetMonthMeterage( 	counter, \
															&(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , \
															counter->maskTarifs , \
															&dtsToRequest.d );
							}
							else
			...
			case TASK_CREATION_STATE_READ_PROFILES:
			{
				...
				if ( res == OK )
				{
					if ( counter->profile == POWER_PROFILE_READ )
					{
						...
						dateTimeStamp dtsToRequest;
						memset(&dtsToRequest , 0 , sizeof(dateTimeStamp) );
						if ( STORAGE_CheckTimeToRequestMeterage( counter ,STORAGE_PROFILE , &dtsToRequest, &canHalfHours) == TRUE )
						{
				...
				if (res == OK)
				{
					...
					if (counter->journalReadingState == STORAGE_JOURNAL_R_STATE_GET_EV_NUMBER)
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = MAYK_AskEventsNumberInCurrentJournal(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , counter->journalNumber );
							else
							{
								res = ERROR_MEM_ERROR;
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
					...
					else if (counter->journalReadingState == STORAGE_JOURNAL_R_STATE_GET_NEXT_EVENT)
					{
						...
						if ( (*counterTask)->transactions != NULL)
						{
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = MAYK_GetNextEventFromCurrentJournal(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , counter->journalNumber , counter->eventNumber);
							else
							{
								res = ERROR_MEM_ERROR;
								COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
			...
			case TASK_CREATION_STATE_READ_PQP:
			{
						...
						if(res == OK)
						{
							...
							if ( (*counterTask)->transactions != NULL)
							{
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = MAYK_GetPowerQualityParametres( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_VOLTAGE );
								else
								{
									res = ERROR_MEM_ERROR;
									COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
						...
						if(res == OK)
						{
							...
							if ( (*counterTask)->transactions != NULL)
							{
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = MAYK_GetPowerQualityParametres( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_AMPERAGE );
								else
								{
									res = ERROR_MEM_ERROR;
									COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
						...
						if(res == OK)
						{
							...
							if ( (*counterTask)->transactions != NULL)
							{
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = MAYK_GetPowerQualityParametres( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER );
								else
								{
									res = ERROR_MEM_ERROR;
									COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
						...
						if(res == OK)
						{
							...
							if ( (*counterTask)->transactions != NULL)
							{
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = MAYK_GetPowerQualityParametres( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_FREQUENCY );
								else
								{
									res = ERROR_MEM_ERROR;
									COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
						...
						if(res == OK)
						{
							...
							if ( (*counterTask)->transactions != NULL)
							{
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = MAYK_GetPowerQualityParametres( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO );
								else
								{
									res = ERROR_MEM_ERROR;
									COMMON_STR_DEBUG(DEBUG_MAYK "MAYK_CreateCounterTask() exit with errorcode=%d", res);
...
int MAYK_SaveTaskResults(counterTask_t * counterTask)
{
	...
	if (counterPtr->ratio != 0xFF){
		ratio = COMMON_GetRatioValueMayak(counterPtr->ratio);
	}
						...
						case TRANSACTION_GET_RATIO:
						{
							...
							if (cmd == ( MAYK_CMD_GET_RATIO | 0x8000 ) )
							{
								if ( (errorStatus == MAYK_RESULT_OK) && (unexpectedAnswerSize == FALSE) )
								{
									...
									if (counterPtr){
										//setup ratio index
										counterPtr->ratio = COMMON_GetMayakRatio(ratio);
										STORAGE_UpdateCounterStatus_Ratio( counterPtr , counterPtr->ratio );
										counterPtr->lastTaskRepeateFlag = 1;
									}
								...
								if ( (errorStatus == MAYK_RESULT_OK) && (unexpectedAnswerSize == FALSE))
								{
									if ( counterPtr )
									{
										...
										if (eventsNumber == 0)
										{
											...
											counterPtr->eventsTotal = 0 ;
											counterPtr->eventNumber = 0 ;

											counterPtr->journalNumber += 1 ;
											if (counterPtr->journalNumber > 6)
												counterPtr->journalNumber = 0 ;
										}
										else
										{
...
void MAYK_PrintfTariff(unsigned char * tariff , int tarifLength)
{
	DEB_TRACE(DEB_IDX_MAYAK)
	...
	if ( MAYK_GetSTT(tariff , tarifLength , &stt , &sttLength) == OK)
	{
		if (stt)
		{
			...
			for( ; i < sttLength ; i++)
			{
				printf("STT%d consists of %d TP's:\n",i , stt[i].tpNumb);
				printf("sec |min |hour|type\n");
				int j = 0 ;
				for( ; j<stt[i].tpNumb ; j++)
				{
					printf(" %02X | %02X | %02X | %02X \n", stt[i].tp[j].ts.s , stt[i].tp[j].ts.m , stt[i].tp[j].ts.h , stt[i].tp[j].tariffNumber);
	...
	maykTariffWTT_t * wtt = NULL;
	int wttLength = 0 ;
	if( MAYK_GetWTT(tariff , tarifLength , &wtt , &wttLength ) == OK )
	{
		if( wtt )
		{
			printf("Entries number = [%d]\n", wttLength);
			...
			for( ; i< wttLength ; i++)
			{
				printf(" %02d | %02d | %02d | %02d \n", wtt[i].weekDaysStamp , wtt[i].weekEndStamp , wtt[i].holidayStamp , wtt[i].shaginMindStamp);
			}
			...
			printf("ERROR: WTT parse OK buf pointer is NULL\n");
	}
	else
		printf("ERROR: Can not parse WTT\n");
	...
	if (MAYK_GetMTT(tariff , tarifLength , &mtt) == OK )
	{
		printf("|JAN |FEB |MAR |APR |MAY |JUN |JUL |AUG |SEP |OCT |NOV |DEC |\n|");
		int i = 0;
		for( ; i<12 ; i++)
		{
			printf(" %02d |", mtt.wttNumb[i]);
		}
		printf("\n");
	}
	else
		printf("ERROR: Can not parse MTT\n");
	...
	if (MAYK_GetLWJ(tariff , tarifLength , &lwj) == OK)
	{
		printf("Entries number = [%d]\n",lwj.mdNumb);
		printf("day |mon |type\n");
		int i = 0 ;
		for(i=0;i<lwj.mdNumb ; i++)
		{
			printf(" %02d | %02d | %02d \n", lwj.md[i].d , lwj.md[i].m , lwj.md[i].t);
		}
	}
	else
		printf("ERROR: Can not parse LWJ\n");
