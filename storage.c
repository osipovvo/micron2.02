...
static pthread_mutex_t storageMuxMap = PTHREAD_MUTEX_INITIALIZER;
			...
			//
				...
				switch (reqIndex){
					...
					case 100:
						...
						//glue
						memset(outputBuff, 0, UD_OUTPUT_SIZE);
						sprintf(outputBuff, "#%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%u||%s||%u||%s||%s||%s",
									uspdSerial,
									uspdEthIp,
									uspdEthMac,
									uspdGsmImei,
...
int STORAGE_GetMaxPossibleTaskTimeForPlc(int nodesNumber){
	DEB_TRACE(DEB_IDX_STORAGE)

	int maxPossibleTime = 0 ;

	if (nodesNumber == 0)
		nodesNumber = STORAGE_MAX_DEVICE_COUNT_PLC;

	maxPossibleTime = (24 * 60 * 60) / nodesNumber / MAX_POOLS_PER_DAY;
	
	COMMON_STR_DEBUG(DEBUG_STORAGE_PLC "STORAGE_GetMaxPossibleTaskTimeForPlc() return maxPossibleTime = %d seconds", maxPossibleTime);
...
int queOffset = 0;
int plcQue[STORAGE_MAX_DEVICE_COUNT_PLC];
double countersWeight[STORAGE_MAX_DEVICE_COUNT_PLC];
...
int STORAGE_PlcQue_ReInitialize(){
	...
	if (countersArrayPlc.countersSize != 0) {
		...
		//reset weight
		for ( i = 0 ; i < STORAGE_MAX_DEVICE_COUNT_PLC ; ++i ){
		...
		for ( j = 0 ; j < countersArrayPlc.countersSize ; ++j)
		{
			...
			for ( i = 0 ; i < countersArrayPlc.countersSize; ++i)
			{
				if ( (k == -1) && ( countersWeight[i] != POLL_WEIGHT_UNKNOWN ) )
				{
					k = i ;
				}
				if ( (countersWeight[i] > maxWeight) && ( countersWeight[i] != POLL_WEIGHT_UNKNOWN )  )
				{
...
int STORAGE_GetNextCounterPlc(counter_t ** counter){
	...
	int maxPlcTime = STORAGE_GetMaxPossibleTaskTimeForPlc(STORAGE_MAX_DEVICE_COUNT_PLC);
	BOOL again = FALSE;
	...
	if (countersArrayPlc.countersSize != 0) {
		...
		if (currentIndexFromQue != -1){
			...
			again = STORAGE_CheckPollingCounterAgainNoAtom(countersArrayPlc.counter[prevCounterIndex], (maxPlcTime + appendTimeout));

			if( again == TRUE ){
				//return the same counter
				*counter = countersArrayPlc.counter[prevCounterIndex];
				countersArrayPlc.currentCounterIndex = prevCounterIndex;
			}
			else
			{
			...
			else
			{
				if ( writingSmsNewParams )
				{
					...
					if ( (port_int > 0) && ( inet_pton(AF_INET, ip, &(sa.sin_addr)) == 1 ) )
					{
						...
						strncpy( (char *)connectionParams_copy.serviceIp, ip , LINK_SIZE );
			...
			if (strstr((char *)strTemp, STORAGE_COUNTER_STRING) != NULL)// find config str for new device
			{
				...
				if (countersArray->counter != NULL) {
					...
					if (countersArray->counter[counterIndex] != NULL) {
						...
						//ratio
						countersArray->counter[counterIndex]->ratio = 0xFF;
						...
						if (countersArray->countersSize > STORAGE_MAX_DEVICE_COUNT_PLC)
							break;
					} else
...
int STORAGE_CheckConfigCountersPlc(char * buf, unsigned int bufLength , int * firstRetValue , int * secRetValue)
{
	...
	//tokenize buffer
	int remoteCounter = 0 ;
	const int tokenNumber = STORAGE_MAX_DEVICE_COUNT_PLC * 13 ;
	char * lastPtr = NULL ;
	int tokenCounter = 0 ;
	...
	char * remotePtr[ STORAGE_MAX_DEVICE_COUNT_PLC ];
	int tokenIndex = 0 ;
			...
			if (strstr((char *)strTemp, STORAGE_COUNTER_STRING) != NULL)// find config str for new device
			{
				...
				if (countersArray->counter != NULL) {
					...
					if (countersArray->counter[counterIndex] != NULL) {
						...
						//check loaded count
						if (countersArray->countersSize > STORAGE_MAX_DEVICE_COUNT_485)
							break;
...
int STORAGE_GetLastMeterageDts(int storage, unsigned long counterDbId, dateTimeStamp * dts) {
	...
	//if (st.st_size != ZERO){
		...
		//old
		//data_in_file = (meterage_t *) malloc(st.st_size);
		if (data_in_file == NULL) {
			//debug
			COMMON_STR_DEBUG(DEBUG_STORAGE_ALL "STORAGE_GetLastMeterageDts ERROR with alloc mem()");
		...
		//old
		//result = read(fd, data_in_file, st.st_size);
		//blocks = result / sizeof (meterage_t);
		...
		//if ((result != ZERO) && (blocks != ZERO)){
			...
			for (blockIndex = 0; blockIndex < blocks; blockIndex++) {
				...
				if (blockCrc == data_in_file[blockIndex].crc){
					...
					//copy stamp of last record
					memcpy(dts, &(data_in_file[blockIndex].dts), sizeof (dateTimeStamp));
					res = OK;
					break;
				}
			}
			...
			//debug
			COMMON_STR_DEBUG(DEBUG_STORAGE_ALL "STORAGE_GetLastMeterageDts READ FILE error ");
...
int STORAGE_GetLastProfileDts(unsigned long counterDbId, dateTimeStamp * dts) {
	...
	//if (st.st_size != ZERO){
		...
		//new
		blocks = st.st_size/ size_of_block;
		data_in_file = (profile_t *) malloc(blocks * size_of_block);

		//old
		//data_in_file = (profile_t *) malloc(st.st_size);
		if (data_in_file == NULL) {
			//debug
			COMMON_STR_DEBUG(DEBUG_STORAGE_ALL "STORAGE_GetLastProfileDts ERROR with alloc mem()");
		...
		//old
		//result = read(fd, data_in_file, st.st_size);
		//blocks = result / sizeof (profile_t);
		...
		//if ((result != ZERO) && (blocks != ZERO)){
			...
			for (blockIndex = 0; blockIndex < blocks; blockIndex++){
				...
				if (blockCrc == data_in_file[blockIndex].crc){
					...
					memcpy(dts, &(data_in_file[blockIndex].dts), sizeof(dateTimeStamp));
					//printf("\n\nOK: wait crc [%08X] , real crc [%08X]\n\n" , blockCrc , data_in_file[blockIndex].crc );
					res = OK;
					break;
				}
				else
				{
					//printf("\n\nERROR: wait crc [%08X] , real crc [%08X]\n\n" , blockCrc , data_in_file[blockIndex].crc );
				}
			}

		} else {
			//debug
			COMMON_STR_DEBUG(DEBUG_STORAGE_ALL "STORAGE_GetLastProfileDts READ FILE error ");
...
BOOL STORAGE_CheckTimeToRequestProfile( counter_t * counter, unsigned int * ptr, dateTimeStamp * dtsToRequest){
	DEB_TRACE(DEB_IDX_STORAGE);
			...
			case STORAGE_PROFILE:
			{
				...
				if ( COMMON_CheckDtsForValid(&lastMetrageDts) == OK  )
				{
					...
					if ((diff > 1) || (mountDflag == 1))
					{
						...
						memcpy(dtsToRequest , &lastMetrageDts , sizeof(dateTimeStamp) );
						COMMON_STR_DEBUG(DEBUG_STORAGE_ALL "STORAGE It is time to ask profile from [" DTS_PATTERN "], canHalfHours [%d]", DTS_GET_BY_PTR(dtsToRequest), diff );
...
void STORAGE_ClearMeterages( unsigned long counterDbId )
{
	...
	int journalIndex = 0 ;
	for( ; journalIndex < 7 ; ++journalIndex )
	{
