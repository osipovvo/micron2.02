	...
	switch (ethState)
	{
		...
		case CONNECTION_READY:
			...
			{				
				...
				//COMMMON_CreateDescriptionForConnectEvent(evDesc , connection);
				memset(evDesc , 0 , EVENT_DESC_SIZE );
				int connectionTime = ETH_GetCurrentConnectionTime();
...
//
int ETH_Close(connection_t * connection __attribute__((unused)))
{
	DEB_TRACE(DEB_IDX_ETH)
