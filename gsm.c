...
BOOL GSM_CheckTimeToConnect( connection_t * connection )
{
	...
	switch( connection->mode )
	{
		...
		{
			if ( incomingCall == GSM_ICS_NEED_TO_ANSWER)
				res = TRUE ;
			else if ( incomingCall == GSM_ICS_NEED_TO_HANG )
			{
				unsigned char * possibleAnswerBank[] = GSM_CMD_ANSWER_BANK_STANDART ;
				unsigned char * answer = NULL ;
				int answerLength = 0 ;
	...
	if ( res == TRUE )
	{
		COMMON_STR_DEBUG( DEBUG_GSM "GSM_CheckTimeToConnect return TRUE" , res );
	}
	...
	switch( connection->mode )
	{
		...
		case CONNECTION_CSD_OUTGOING:
		{
			if ( gsmNeedToClose == TRUE )
			{
				...
				int connectionTime = GSM_GetCurrentConnectionTime();
...
int GSM_Open( connection_t * connection )
{
	...
	switch( connection->mode )
	{
		...
		case CONNECTION_GPRS_CALENDAR:
		{
			COMMON_STR_DEBUG(DEBUG_GSM "GSM_Open() creating connection profile for grps connection");
			unsigned char * possibleAnswerBank[] = GSM_CMD_ANSWER_BANK_STANDART ;
...
//							if ( serviceMode == TRUE )
//							{
//								COMMON_STR_DEBUG( DEBUG_GSM "GSM creating service profile: set addres [%s] : [%u] OK" , connection->serviceIp , connection->servicePort);
//							}
//							else
//							{
//								COMMON_STR_DEBUG( DEBUG_GSM "GSM creating service profile: set addres [%s] : [%u] OK" , connection->server , connection->port);
//							}
...
int GSM_Close(connection_t * connection)
{
	...
	switch ( connection->mode )
	{
		...
		case CONNECTION_GPRS_CALENDAR:
		{
			...
			int answerLength = 0 ;
			unsigned char * possibleAnswerBank[] = GSM_CMD_ANSWER_BANK_STANDART ;
			if ( GSM_CmdWrite((unsigned char *)"AT^SISC=0\r" ,FALSE, GSM_SMALL_ANSWER_TIMEOUT , possibleAnswerBank , &answer , &answerLength) == OK )
			{
				if ( answer )
				{
					if ( strstr( (char *)answer , "\r\nOK\r\n" ) )
		...
		case CONNECTION_GPRS_CALENDAR:
		{
			if ( GSM_ConnectionState() == OK )
			{
				...
				if ( gsmReadyToWrite == TRUE )
				{
					...
					unsigned char cmd[64];
					memset( cmd , 0 , 64);
					snprintf( (char *)cmd , 64 , "AT^SISW=0,%d\r" , size );
	...
	switch ( connection->mode )
	{
		...
		case CONNECTION_GPRS_CALENDAR:
		{
			if ( GSM_ConnectionState() == OK )
			{
				...
				unsigned char cmd[64];
				memset( cmd , 0 , 64);
				snprintf( (char *)cmd , 64 , "AT^SISR=0,%d\r" , size);
	...
	switch ( connection->mode )
	{
		...
		case CONNECTION_GPRS_CALENDAR:
		{
			if ( GSM_ConnectionState() == OK )
			{
				...
				{
					COMMON_STR_DEBUG( DEBUG_GSM "GSM_ReadAvailable() asking for bytes available to read");
					...
					int answerLength = 0 ;
					unsigned char * possibleAnswerBank[] = GSM_CMD_ANSWER_BANK_STANDART ;
					if ( GSM_CmdWrite((unsigned char *)"AT^SISR=0,0\r" ,FALSE, GSM_SMALL_ANSWER_TIMEOUT , possibleAnswerBank , &answer , &answerLength) == OK )
					{
						if ( answer )
						{
							if ( strstr( (char *)answer , "\r\nOK\r\n" ) )
							{
								//parsing
								unsigned char * startPos = (unsigned char *)strstr((char *)answer , "AT^SISR=0,0\r");
								if( startPos )
								{
									const int tokenNumber = 10 ;
									int tokenCounter = 0 ;
...
void GSM_AsyncMessageHandler(unsigned char * foundedPhrase, connection_t * connection)
{
	...
	{
		COMMON_STR_DEBUG( DEBUG_GSM "GSM Async message ^SIS was found" , foundedPhrase );
