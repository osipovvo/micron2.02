...
int UJUK_Command_GetProfilePointer(counter_t * counter, counterTransaction_t ** transaction, dateTimeStamp *dts)
{
	...
	//profile time
	tempCommand[commandSize++] = (unsigned char)30;
...
//
//
int UJUK_Command_GetProfileAllHour(counter_t * counter, counterTransaction_t ** transaction, unsigned int pointer)
{
	DEB_TRACE(DEB_IDX_UJUK)
...
int UJUK_GetPowerQualityParametres(counter_t * counter, counterTransaction_t ** transaction , int pqpPart , int phaseNumber)
{
	DEB_TRACE(DEB_IDX_UJUK)
...
int UJUK_SyncTime(counter_t * counter, counterTask_t ** counterTask )
{
	...
	if ( abs(counter->syncWorth) > UJUK_SOFT_HARD_SYNC_SW )
	{
		...
		if ((*counterTask)->transactions != NULL) {
			...
			if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
			{
				...
				res = UJUK_Command_SyncTimeHardy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
			}
			...
			if ( (counter->profileInterval != STORAGE_UNKNOWN_PROFILE_INTERVAL) && (!(60 % counter->profileInterval)) )
			{
				...
				for ( ; stepIndex < stepTotal ; ++stepIndex )
				{
					...
					if ((*counterTask)->transactions != NULL) {
						...
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
						{
							res = UJUK_Command_SetProfileIntervalValue(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , counter->profileInterval );
						}
						else
							return ERROR_MEM_ERROR;
					} else
	...
	else
	{
		...
		if ((*counterTask)->transactions != NULL) {
			...
			{
				STORAGE_SwitchSyncFlagTo_SyncProcess(&counter);
				res = UJUK_Command_SyncTimeSoft(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
			}
...
int UJUK_CreateProfileTr( counter_t * counter, counterTask_t ** counterTask )
{
	...
	(*counterTask)->resultProfile.ratio = counter->ratio & 0x0f;
	...
	while( readProfileState != STATE_END )
	{
		switch(readProfileState)
		{
			...
			case STATE_CHECK_TIME_TO_REQUEST:
				...
				if ( STORAGE_CheckTimeToRequestProfile(counter, &ptrToRead, &counter->profileLastRequestDts, NULL) == TRUE )
				{
			...
			case STATE_SEARCH_PTR:
				...
				if ((*counterTask)->transactions != NULL) {
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
						//res = UJUK_Command_GetProfilePointer(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &(*counterTask)->resultProfile.dts );
						res = UJUK_Command_GetProfilePointer(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , &counter->profileLastRequestDts);
					} else
						return ERROR_MEM_ERROR;
				} else
					return ERROR_MEM_ERROR;
			...
			case STATE_ASK_SEARCH_STATE:
				...
				if ((*counterTask)->transactions != NULL) {
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
						res = UJUK_Command_GetProfileSearchState(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) ;
					} else
						return ERROR_MEM_ERROR;
				} else
					return ERROR_MEM_ERROR;
			...
			case STATE_READ_BY_PTR:
				...
				COMMON_STR_DEBUG(DEBUG_NEWPROFILE "PROFILE TR CREATE : READ BY PTR"); //new profile debug

				(*counterTask)->transactions = (counterTransaction_t **) realloc((*counterTask)->transactions, (++(*counterTask)->transactionsCount) * sizeof (counterTransaction_t *));
				if ((*counterTask)->transactions != NULL) {
					(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
						res = UJUK_Command_GetProfileAllHour(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , ptrToRead) ;
						...
						if (dataParsingResultLength == commandsCount){
							...
							switch (pwdLevel){
								case 1:
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL){
											res = UJUK_Command_OpenCounter(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
										} else {
											res = ERROR_MEM_ERROR;
										}
									} else {
								...
								case 2:
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL){
											res = UJUK_Command_OpenCounterForWritingToPhMemmory (counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
										} else {
											res = ERROR_MEM_ERROR;
										}
									} else {
							...
							for( searchIndex = 0 ; ((searchIndex < dataParsingResultLength) && (res == OK)) ; ++searchIndex )
							{
								...
								if ((*counterTask)->transactions != NULL) {
									...
									if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									{
										res = UJUK_Command_UserCommand(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , dataParsingResult[ searchIndex ].value);
										if (res == OK){
											(*counterTask)->userCommandsTransactionCounter++;
										}
									}
									else
										res = ERROR_MEM_ERROR;
...
int UJUK_WriteTariffMap(counter_t * counter, counterTask_t ** counterTask ,unsigned char * tariff, int tarifLength)
{
	...
	{
		res = ERROR_FILE_FORMAT ;
		if ( (counter->type == TYPE_UJUK_PSH_4TM_05MK) || (counter->type == TYPE_UJUK_SEB_1TM_02M))
		{
						...
						if ( res == OK )
						{
							...
							if (fields == 17)
							{
								...
								if ((*counterTask)->transactions != NULL) {
									...
									if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									{
										res = UJUK_Command_SetTariffZone(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] ,
												&ujukTariffZone , blockParsingIndex );
							...
							while(1)
							{
								...
								if (res == OK) {
									...
									if ((*counterTask)->transactions != NULL) {
										...
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										{
											res = UJUK_Command_WriteDataToPhisicalMemory(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , \
			...
			if (res == OK) {
				...
				if ((*counterTask)->transactions != NULL) {
					...
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
					{
						...
						//debug
//						printf( "holiday %04X:\t", startHolidayAddress );
//						int i = 0 ;
//						for( i = holidayArrPos ; i < bytesNumb + holidayArrPos; i++)
//							printf("%02X " , ujukHolidayArr.data[ i ]);
//						printf("\r\n");
					}
		...
		if (res == OK) {
			...
			if ((*counterTask)->transactions != NULL) {
				...
				if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
				{
					...
					//debug
//					printf("holiday crc %04X:\t%02X" , startHolidayAddress , ujukHolidayArr.crc );
				}
				else
					return ERROR_MEM_ERROR;
			} else
			...
			if (res == OK) {
				...
				if ((*counterTask)->transactions != NULL) {
					...
					if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
					{
						...
						//debug
//						printf( "tarif %04X:\t", startTariffMapAddr );
//						int i = 0 ;
//						for( i = bufPos ; i < bytesNumb + bufPos; i++)
//							printf("%02X " , buf[ i ]);
//						printf("\r\n");
					...
					if (res == OK) {
						...
						if ((*counterTask)->transactions != NULL) {
							...
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							{
								...
								//debug
//								printf( "moved %04X:\t", startMovedDayAddr );
//								int i = 0 ;
//								for( i = bufPos ; i < bytesNumb + bufPos; i++)
//									printf("%02X " , buf[ i ]);
//								printf("\r\n");
					...
					if ( !strcmp( blockParsingResult[ blockParsingResultIndex ].variable , "CMD_0") )
					{
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								...
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								{
									res = UJUK_Command_SetIndicationGeneral(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , param);
					...
					else if ( strstr( blockParsingResult[ blockParsingResultIndex ].variable , "IndExtParams_") )
					{
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								...
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								{
									res = UJUK_Command_SetIndicationExtended(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , param);
									(*counterTask)->indicationWritingTransactionCounter++;
								}
								else
									return ERROR_MEM_ERROR;
					...
					else if ( !strcmp( blockParsingResult[ blockParsingResultIndex ].variable , "IndPrd") )
					{
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								...
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								{
									res = UJUK_Command_SetIndicationPeriod(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , period);
									(*counterTask)->indicationWritingTransactionCounter++;
								}
								else
									return ERROR_MEM_ERROR;
					...
					else if ( !strcmp( blockParsingResult[ blockParsingResultIndex ].variable , "AllowDynInd") )
					{
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								...
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								{
									res = UJUK_Command_SetDynamicIndicationProperties(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , property_byte);
									(*counterTask)->indicationWritingTransactionCounter++;
								}
								else
									return ERROR_MEM_ERROR;
									...
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
											{
												...
												res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0 , NULL , NULL );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								if ( atoi( blockParsingResult[ propertyIndex ].value) == 1 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 1 , NULL , NULL );
											}
											else
												return ERROR_MEM_ERROR;
										} else
							...
							if ( strstr( blockParsingResult[ propertyIndex ].variable , "mainsUpperThresholdVoltage") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x09 , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "mainsLowerThresholdVoltage") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x0D , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "mainsThresholdVoltageHysteresis") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x0A , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "mainsAveragingPeriodsQty") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x0B , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "powerOnLagAfterVoltageReturnToGivenLimits") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x0C , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 1 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x3A  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x39  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 1 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x51  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x50  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
							...
							else if ( strstr( blockParsingResult[ propertyIndex ].variable , "totalEnergy") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x0E , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
							...
							if ( strstr( blockParsingResult[ propertyIndex ].variable , "allowLoadControlAtDailyEnergyLimitOvershoot") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x07 , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 1 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x20  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x21  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 1 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x53  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x52  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
									...
									case TYPE_UJUK_SEB_1TM_02M:
									{
										...
										if ( res == OK )
										{
											...
											if ((*counterTask)->transactions != NULL) {
												...
												{
													((*counterTask)->powerControlTransactionCounter)++;
													res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x04 , &tariffNumb , &propValue );
												}
												else
													return ERROR_MEM_ERROR;
											} else
							...
							if ( strstr( blockParsingResult[ propertyIndex ].variable , "onSaturday") )
							{
								...
								if ( res == OK )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										...
										{
											((*counterTask)->powerControlTransactionCounter)++;
											res = UJUK_Command_SetPowerConterolRules( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x08 , &tariffNumb , &propValue );
										}
										else
											return ERROR_MEM_ERROR;
									} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 0 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x4B  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x4A  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 0 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x31  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x32  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 0 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x37  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x38  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
				...
				if ( ( powerThreshold >= 0 ) && ( averagingTimeForPowerThreshold >= 0 ) )
				{
					...
					if ( res == OK )
					{
						...
						if ((*counterTask)->transactions != NULL) {
							...
							{
								((*counterTask)->powerControlTransactionCounter)++;
								res = UJUK_Command_SetPowerLimitControls( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , byteArr  );
							}
							else
								return ERROR_MEM_ERROR;
						} else
								...
								if ( atoi(blockParsingResult[ propertyIndex ].value) == 0 )
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x2F  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
								...
								else
								{
									if ( res == OK )
									{
										...
										if ((*counterTask)->transactions != NULL) {
											...
											{
												((*counterTask)->powerControlTransactionCounter)++;
												res = UJUK_Command_SetControlProgrammFlags( counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , 0x30  );
											}
											else
												return ERROR_MEM_ERROR;
										} else
			...
			else
			{
				...
				if ( res == OK )
				{
					...
					if ((*counterTask)->transactions != NULL) {
						(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							res = UJUK_Command_GetCounterStateWord (counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
						else
							return ERROR_MEM_ERROR;
					} else
						return ERROR_MEM_ERROR;
					...
					if (  allowSeasonFlg != counterAllowSeason )
					{
						...
						if ( res == OK )
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
									res = UJUK_Command_SetAllowSeasonChangeFlg(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], allowSeasonFlg);
								else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if ( allowSeasonFlg == STORAGE_SEASON_CHANGE_ENABLE )
						{
							if (res == OK )
							{
								...
								if ((*counterTask)->transactions != NULL) {
									(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
									if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										res = UJUK_Command_SetCurrentSeason(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , currentSeason) ;
									else
										return ERROR_MEM_ERROR;
								} else
									return ERROR_MEM_ERROR;
			...
			case TASK_CREATION_STATE_READ_RATIO_DATE_AND_TIME:
			{
				...
				if (res == OK) {
					...
					if ((*counterTask)->transactions != NULL) {
						(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
						if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
							res = UJUK_Command_GetDateAndTime(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
						else
							return ERROR_MEM_ERROR;
					} else
						return ERROR_MEM_ERROR;
				...
				if (res == OK)  {
...
#if 0
					if ( COMMON_CheckRatioIndexForValid( counter->type , counter->ratio ) == FALSE )
					{
						if ( counter->ratio != 0xFF )
						{
							//save journal event
							COMMON_STR_DEBUG( DEBUG_UJUK "UJUK Incorrect ratio index value [%hhu]", counter->ratio );
							unsigned char evDesc[EVENT_DESC_SIZE];
							unsigned char dbidDesc[EVENT_DESC_SIZE];
							memset(dbidDesc , 0, EVENT_DESC_SIZE);
							COMMON_TranslateLongToChar(counter->dbId, dbidDesc);
							...
							case TYPE_UJUK_SEB_1TM_02M:
								...
								if ( counter->profileInterval == STORAGE_UNKNOWN_PROFILE_INTERVAL )
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_Command_GetProfileIntervalValue(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] );
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if ((*counterTask)->transactions != NULL) {
									(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
									if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
										res = UJUK_Command_GetProfileCurrentPtr(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
									else
										return ERROR_MEM_ERROR;
								} else
									return ERROR_MEM_ERROR;
					...
					if (counter->ratio == 0xFF){
						...
						if ((*counterTask)->transactions != NULL) {
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
								res = UJUK_Command_GetRatioValue(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1]);
							else
								return ERROR_MEM_ERROR;
						} else
							return ERROR_MEM_ERROR;
							...
							switch( counter->type )
							{
								...
								case TYPE_UJUK_SEB_1TM_02M:
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
											res = UJUK_Command_GetMeteragesByBinaryMask(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], UJUK_BINARY_MASK_METERAGE_ARCH_TYPE_CURRENT , 0);
										else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								default:
								{
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T1)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
												res = UJUK_Command_CurrentMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_1);
											else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T2)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
												res = UJUK_Command_CurrentMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_2);
											else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T3)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
												res = UJUK_Command_CurrentMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_3);
											else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T4)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL)
												res = UJUK_Command_CurrentMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_4);
											else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
								...
								case TYPE_UJUK_SEB_1TM_02M:
								{
									...
									if (res == OK)
									{
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_GetMeteragesByBinaryMask(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], UJUK_BINARY_MASK_METERAGE_ARCH_TYPE_MONTH, askMonthIndex);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									for ( ; monthIndex < monthesDiff ; ++monthIndex )
									{
										...
										if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T1)) {
											...
											if ((*counterTask)->transactions != NULL) {
												(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
												if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
													res = UJUK_Command_MonthEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_1, askMonthIndex);
										...
										if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T2)) {
											...
											if ((*counterTask)->transactions != NULL) {
												(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
												if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
													res = UJUK_Command_MonthEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_2, askMonthIndex);
													(*counterTask)->transactions[ (*counterTask)->transactionsCount - 1 ]->monthEnergyRequest = monthIndex ;
												} else
													return ERROR_MEM_ERROR;
											} else
										...
										if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T3)) {
											...
											if ((*counterTask)->transactions != NULL) {
												(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
												if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
													res = UJUK_Command_MonthEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_3, askMonthIndex);
													(*counterTask)->transactions[ (*counterTask)->transactionsCount - 1 ]->monthEnergyRequest = monthIndex ;
												} else
													return ERROR_MEM_ERROR;
											} else
										...
										if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T4)) {
											...
											if ((*counterTask)->transactions != NULL) {
												(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
												if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
													res = UJUK_Command_MonthEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_4, askMonthIndex);
													(*counterTask)->transactions[ (*counterTask)->transactionsCount - 1 ]->monthEnergyRequest = monthIndex ;
												} else
													return ERROR_MEM_ERROR;
											} else
								...
								default:
								{
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T1)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_MonthMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_1, askMonthIndex);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T2)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_MonthMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_2, askMonthIndex);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T3)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_MonthMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_3, askMonthIndex);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T4)) {
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_MonthMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_4, askMonthIndex);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
						...
						else
						{
							...
							if ( STORAGE_CheckTimeToRequestMeterage( counter , STORAGE_DAY , &dtsToRequest ) == TRUE )
							{
								...
								switch( counter->type )
								{
									...
									case TYPE_UJUK_SEB_1TM_02M:
									{
										//ask only today or yesterday meterages

										int memoryType = UJUK_BINARY_MASK_METERAGE_ARCH_TYPE_YESTERDAY ;
										if ( daysDiff == 1 )
										{
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_GetMeteragesByBinaryMask(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], memoryType, 0);
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
									...
									case TYPE_UJUK_SET_4TM_03M:
									{
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_Command_GetMeteragesByBinaryMask(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], UJUK_BINARY_MASK_METERAGE_ARCH_TYPE_DAY, dtsToRequest.d.d );
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
										//ask day archive for 30 days
									}
									break;
											...
											if ( askDayMode == ASK_DAY_YESTERDAY )
											{
												...
												if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T1)) {
													...
													if ((*counterTask)->transactions != NULL) {
														(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
														if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
															res = UJUK_Command_DayEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_1, askDayMode );
														} else
															return ERROR_MEM_ERROR;
													} else
														return ERROR_MEM_ERROR;
												...
												if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T2)) {
													...
													if ((*counterTask)->transactions != NULL) {
														(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
														if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
															res = UJUK_Command_DayEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_2, askDayMode );
														} else
															return ERROR_MEM_ERROR;
													} else
														return ERROR_MEM_ERROR;
												...
												if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T3)) {
													...
													if ((*counterTask)->transactions != NULL) {
														(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
														if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
															res = UJUK_Command_DayEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_3, askDayMode );
														} else
															return ERROR_MEM_ERROR;
													} else
														return ERROR_MEM_ERROR;
												...
												if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T4)) {
													...
													if ((*counterTask)->transactions != NULL) {
														(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
														if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
															res = UJUK_Command_DayEnergy(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_4, askDayMode );
														} else
															return ERROR_MEM_ERROR;
													} else
														return ERROR_MEM_ERROR;
										...
										if (askDayMode != ASK_EMPTY)
										{
											...
											if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T1)) {
												...
												if ((*counterTask)->transactions != NULL) {
													(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
													if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
														res = UJUK_Command_DayMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_1, askDayMode, askDayIndex);
													} else
														return ERROR_MEM_ERROR;
												} else
													return ERROR_MEM_ERROR;
											...
											if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T2)) {
												...
												if ((*counterTask)->transactions != NULL) {
													(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
													if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
														res = UJUK_Command_DayMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_2, askDayMode, askDayIndex);
													} else
														return ERROR_MEM_ERROR;
												} else
													return ERROR_MEM_ERROR;
											...
											if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T3)) {
												...
												if ((*counterTask)->transactions != NULL) {
													(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
													if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
														res = UJUK_Command_DayMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_3, askDayMode, askDayIndex);
													} else
														return ERROR_MEM_ERROR;
												} else
													return ERROR_MEM_ERROR;
											...
											if ((res == OK) && (counter->maskTarifs & COUNTER_MASK_T4)) {
												...
												if ((*counterTask)->transactions != NULL) {
													(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
													if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
														res = UJUK_Command_DayMeterage(counter, &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1], TARIFF_4, askDayMode, askDayIndex);
													} else
														return ERROR_MEM_ERROR;
												} else
													return ERROR_MEM_ERROR;
					...
					if (res == OK)
					{
						...
						if (counter->journalReadingState == STORAGE_JOURNAL_R_STATE_GET_NEXT_EVENT)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_Command_GetNextEventFromCurrentJournal(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
			...
			case TASK_CREATION_STATE_READ_PQP:
			{
				...
				switch( counter->type )
				{
					...
					case TYPE_UJUK_SEB_1TM_02M:
					{
						if ( counter->profileReadingState != STORAGE_PROFILE_STATE_WAITING_SEARCH_RES)
						{
							if ( res == OK)
							{
								...
								else
								{
									...
									if (res == OK)
									{
										...
										if ((*counterTask)->transactions != NULL) {
											(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
											if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
												res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_TAN , phaseNumber ) ;
											} else
												return ERROR_MEM_ERROR;
										} else
											return ERROR_MEM_ERROR;
								...
								if ((*counterTask)->transactions != NULL) {
									(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
									if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
										res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_VOLTAGE , phaseNumber ) ;
									} else
										return ERROR_MEM_ERROR;
								} else
									return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_AMPERAGE , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_P , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_Q , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_S , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_COS , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
								...
								if (res == OK)
								{
									...
									if ((*counterTask)->transactions != NULL) {
										(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
										if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
											res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_FREQUENCY , phaseNumber ) ;
										} else
											return ERROR_MEM_ERROR;
									} else
										return ERROR_MEM_ERROR;
						...
						{
							COMMON_STR_DEBUG( DEBUG_UJUK "UJUK Counter is searching for profile. Do not ask for PQP");
						}
					}
						break;
					...
					case TYPE_UJUK_SET_4TM_02A:
					{
						...
						if ((*counterTask)->transactions != NULL) {
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
								res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_VOLTAGE , 1 ) ;
							} else
								return ERROR_MEM_ERROR;
						} else
							return ERROR_MEM_ERROR;
						...
						if ((*counterTask)->transactions != NULL) {
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
								res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_VOLTAGE , 2 ) ;
							} else
								return ERROR_MEM_ERROR;
						} else
							return ERROR_MEM_ERROR;
						...
						if ((*counterTask)->transactions != NULL) {
							(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
							if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
								res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_VOLTAGE , 3 ) ;
							} else
								return ERROR_MEM_ERROR;
						} else
							return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_AMPERAGE , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_AMPERAGE , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_AMPERAGE , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_P , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_P , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_P , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_P , 0 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_Q , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_Q , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_Q , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_Q , 0 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_S , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_S , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_S , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_POWER_S , 0 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_COS , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_COS , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_COS , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_TAN , 1 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_TAN , 2 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_RATIO_TAN , 3 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
						...
						if (res == OK)
						{
							...
							if ((*counterTask)->transactions != NULL) {
								(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] = (counterTransaction_t *) malloc(sizeof (counterTransaction_t));
								if (((*counterTask)->transactions[(*counterTask)->transactionsCount - 1]) != NULL) {
									res = UJUK_GetPowerQualityParametres(counter , &(*counterTask)->transactions[(*counterTask)->transactionsCount - 1] , PQP_STATE_READING_FREQUENCY , 0 ) ;
								} else
									return ERROR_MEM_ERROR;
							} else
								return ERROR_MEM_ERROR;
		...
		if (transactionResult == TRANSACTION_DONE_OK) {
			...
			switch (transactionType) {
				...
				{					 
					...
					unsigned char ratioIndex = counterTask->transactions[transactionIndex]->answer[aIndex] & RATIO_MASK;
				...
				case TRANSACTION_UJUK_CURR_PPTR:
				{
					...
					if ( counterPtr )
					{
						if ( counterPtr->profileInterval != STORAGE_UNKNOWN_PROFILE_INTERVAL )
						{
							...
							COMMON_STR_DEBUG(DEBUG_UJUK "UJUK current ptr [%u] overflow flg [%hhu]", counterPtr->profileCurrPtr, counterPtr->profilePtrCurrOverflow);
						}
				...
				case TRANSACTION_PROFILE_GET_SEARCHING_STATE:
				{
					...
					switch( counterTask->transactions[transactionIndex]->answer[ aIndex ] )
					{
						...
						case 0x02:
							...
							int pri = 0 ;
							for ( pri =0 ; pri < ( 60 / counterPtr->profileInterval ); pri++)
							{
				...
				case TRANSACTION_GET_EVENT:
				{
					...
					//{
						if (counterPtr)
						{
							...
							if (counterTask->transactions[transactionIndex]->answer[0] == UJUK_LONG_ADDRESS_START_BYTE)
								aIndex += 4;
							unsigned char journalNumber = UJUK_TranslateJournalNumber(counterPtr) ;
							switch(journalNumber)
							{
								case 0:
								{
									event1.evType = EVENT_OPENING_BOX ;
									event1.dts.t.s = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event1.dts.t.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event1.dts.t.h = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );

									aIndex++ ;

									event1.dts.d.d = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event1.dts.d.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event1.dts.d.y = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );

									event2.evType = EVENT_CLOSING_BOX ;

									event2.dts.t.s = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event2.dts.t.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event2.dts.t.h = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );

									aIndex++ ;

									event2.dts.d.d = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event2.dts.d.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									event2.dts.d.y = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
								...
								case 3:
								{
									...
									lastTime.t.s = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									lastTime.t.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									lastTime.t.h = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );

									aIndex++ ;

									lastTime.d.d = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									lastTime.d.m = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
									lastTime.d.y = (unsigned char)COMMON_Translate2_10to10( (int)counterTask->transactions[transactionIndex]->answer[aIndex++] );
								...
								case 6:
								{
									...
									unsigned char reason  = counterTask->transactions[transactionIndex]->answer[aIndex++] ;
									unsigned char state  = counterTask->transactions[transactionIndex]->answer[aIndex++] ;
									if (state == 1)
									{
				...
				case TRANSACTION_GET_PQP_AMPERAGE_PHASE1:
				{
					...
					else
					{
						...
						int Kc = 0 ;
						int Ci = 0 ;
						UJUK_GetTransformationRatioByType( counterTask->counterType , &Kc , &Ci);
				...
				case TRANSACTION_UJUK_WRITE_TARIFF_ZONE + 9 :				
				{
					if ( counterTask->transactions[transactionIndex]->shortAnswerFlag == FALSE )
					{
						...
						if ( ( transactionType - TRANSACTION_UJUK_WRITE_TARIFF_ZONE ) == (trId % 10) )
						{
							...
							//
							// do not check result byte. it can be not zero.
							//
							if ( tariffMapTransactionCounter >= 0 )
							{
								tariffMapTransactionCounter++ ;
		...
		default:
		{
			if ( (resultEnergyToday == READY) && (counterTask->resultCurrentReady == READY))
			{
				...
				if ( resultEnergyYesterday == READY )
				{
					...
					int tarifIndex = 1 ;
					for( tarifIndex = 0x01 ; tarifIndex <= 0xF && tarifIndex>0 ; tarifIndex = tarifIndex << 1)
					{
						if ( (tarifIndex & resultMetrCurrMsk) )
						{
							counterTask->resultDay.t[ tarifIndex ].e[ ENERGY_AP ] = counterTask->resultCurrent.t[ tarifIndex ].e[ ENERGY_AP ] - dayEnergyToday.t[ tarifIndex ].e[ ENERGY_AP ] - dayEnergyYesterday.t[ tarifIndex ].e[ ENERGY_AP ];
							counterTask->resultDay.t[ tarifIndex ].e[ ENERGY_AM ] = counterTask->resultCurrent.t[ tarifIndex ].e[ ENERGY_AM ] - dayEnergyToday.t[ tarifIndex ].e[ ENERGY_AM ] - dayEnergyYesterday.t[ tarifIndex ].e[ ENERGY_AM ];
							counterTask->resultDay.t[ tarifIndex ].e[ ENERGY_RP ] = counterTask->resultCurrent.t[ tarifIndex ].e[ ENERGY_RP ] - dayEnergyToday.t[ tarifIndex ].e[ ENERGY_RP ] - dayEnergyYesterday.t[ tarifIndex ].e[ ENERGY_RP ];
							counterTask->resultDay.t[ tarifIndex ].e[ ENERGY_RM ] = counterTask->resultCurrent.t[ tarifIndex ].e[ ENERGY_RM ] - dayEnergyToday.t[ tarifIndex ].e[ ENERGY_RM ] - dayEnergyYesterday.t[ tarifIndex ].e[ ENERGY_RM ];
						}
					}
					//saving meterage
					if ( COMMON_CheckSawInMeterage( &counterTask->resultDay ) == OK )
				...
				if ( saveTodayMeterage == TRUE )
				{
					...
					{
						counterTask->resultDay.t[ tariffIndex ].e[ ENERGY_AP ] = counterTask->resultCurrent.t[ tariffIndex ].e[ ENERGY_AP ] - dayEnergyToday.t[ tariffIndex ].e[ ENERGY_AP ] ;
						counterTask->resultDay.t[ tariffIndex ].e[ ENERGY_AM ] = counterTask->resultCurrent.t[ tariffIndex ].e[ ENERGY_AM ] - dayEnergyToday.t[ tariffIndex ].e[ ENERGY_AM ] ;
						counterTask->resultDay.t[ tariffIndex ].e[ ENERGY_RP ] = counterTask->resultCurrent.t[ tariffIndex ].e[ ENERGY_RP ] - dayEnergyToday.t[ tariffIndex ].e[ ENERGY_RP ] ;
						counterTask->resultDay.t[ tariffIndex ].e[ ENERGY_RM ] = counterTask->resultCurrent.t[ tariffIndex ].e[ ENERGY_RM ] - dayEnergyToday.t[ tariffIndex ].e[ ENERGY_RM ] ;
					}
				...
				for ( ; monthIndex < counterTask->monthRequestCount ; ++monthIndex )
				{
					...
					for ( ; offsetIndex < counterTask->monthRequestCount ; ++offsetIndex )
					{
						if ( resultEnergyMonth[ offsetIndex ] == READY )
						{
							...
							for( ; tariffIndex < MAX_TARIFF_NUMBER ; ++tariffIndex )
							{
								counterTask->resultMonth.t[ tariffIndex ].e[ ENERGY_AP ] -= monthEnergy[ offsetIndex ].t[ tariffIndex ].e[ ENERGY_AP ] ;
								counterTask->resultMonth.t[ tariffIndex ].e[ ENERGY_AM ] -= monthEnergy[ offsetIndex ].t[ tariffIndex ].e[ ENERGY_AM ] ;
								counterTask->resultMonth.t[ tariffIndex ].e[ ENERGY_RP ] -= monthEnergy[ offsetIndex ].t[ tariffIndex ].e[ ENERGY_RP ] ;
								counterTask->resultMonth.t[ tariffIndex ].e[ ENERGY_RM ] -= monthEnergy[ offsetIndex ].t[ tariffIndex ].e[ ENERGY_RM ] ;
							}
						}
					}
...
void UJUK_GetTransformationRatioByType( unsigned char counterType , int * Kc , int * Ci )
{
	switch( counterType )
	{
		case TYPE_UJUK_SET_4TM_02A:
...
unsigned char UJUK_TranslateJournalNumber(counter_t * counter)
{
	DEB_TRACE(DEB_IDX_UJUK)
...
void UJUK_PrintfTariff(unsigned char * tariff , int tarifLength)
{
	...
	printf("UJUK_PrintfTariff() start\r\n");
	ujukHolidayArr_t ujukHolidayArr ;
	memset(&ujukHolidayArr , 0 , sizeof(ujukHolidayArr_t));

	int res = UJUK_GetHolidayArray( &ujukHolidayArr , tariff , tarifLength) ;
	if ( res == OK )
	{
		printf("\t[Holiday]\r\nholiday array: ");
		int idx = 0 ;
		for( ; idx < UJUK_TARIFF_HOLIDAY_ARR_MAX_SIZE ; idx++)
			printf("%02X " , ujukHolidayArr.data[idx]);
		printf("\r\nholiday crc = [%02X]\r\n" , ujukHolidayArr.crc);
	}
	else
		printf("ERROR - UJUK_GetHolidayArray returned [%d]\r\n" , res );
	...
	if (res == OK )
	{
		printf("\t[Moved days]\r\nlist contains [%d] elements: " , ujukMovedDaysList.dataSize);
		int idx = 0 ;
		for( ; idx < ujukMovedDaysList.dataSize ; idx++)
			printf("%08X ", ujukMovedDaysList.data[idx]);
		printf("\r\nmoved days crc = [%02X]\r\n" , ujukMovedDaysList.crc);
	}
	else
		printf("ERROR - UJUK_GetMovedDaysList returned [%d]\r\n" , res );

	ujukTariffMap_t ujukTariffMap ;
	int tariffNumber ;
	res = UJUK_GetTariffMap(&ujukTariffMap , tariff , tarifLength , &tariffNumber);
	if (res == OK)
	{
		printf("\t[Tariff map]:\r\n");
		int monthIndex = 0 ;
		...
		for( ; monthIndex < 12 ; monthIndex++)
		{
			printf("\r\nMonth N[%d] contains [%d] DayTypes\r\n" , monthIndex + 1, ujukTariffMap.monthMap[monthIndex].dayNumb);

			int dayIndex = 0 ;

			for( ; dayIndex < ujukTariffMap.monthMap[monthIndex].dayNumb ; dayIndex++)
			{
				printf("day N [%d] :" , dayIndex);

				int dataIndex = 0 ;

				for( ; dataIndex < ujukTariffMap.monthMap[monthIndex].day[dayIndex].dataSize ; dataIndex++)
				{
					printf("%02d ", ujukTariffMap.monthMap[monthIndex].day[dayIndex].data[dataIndex]);
				}
				printf("\r\n\r\n");
			}
		...
		printf("ERROR - UJUK_GetTariffMap returned [%d]\r\n" , res );
}
